...
enum {
    ...
    CASET = 0x2A, RASET = 0x2B, RAMWR = 0x2C, COLMOD = 0x3A, TEON = 0x35,
    MADCTL = 0x36, PORCTRL = 0xB2, GCTRL = 0xB7, VCOMS = 0xBB, LCMCTRL = 0xC0,
    VDVVRHEN = 0xC2, VRHS = 0xC3, VDVS = 0xC4, FRCTRL2 = 0xC6, PWCTRL1 = 0xD0,
    RAMCTRL = 0xB0, GMCTRP1 = 0xE0, GMCTRN1 = 0xE1
};

enum { MADCTL_COL = 0x40, MADCTL_ROW = 0x80, MADCTL_SWAP_XY = 0x20, MADCTL_SCAN = 0x10 };
...
#define CMDN(c, ...) do { static const uint8_t _d[] = {__VA_ARGS__}; \
                          cmd((c), sizeof(_d), _d); } while (0)
...
static void pio_dma_setup(void) {
    ...
    for (int i = 0; i < 8; i++) pio_gpio_init(pio, LCD_D0 + i);
...
void lcd_init(void) {
    ...
    gpio_init(POWER_EN); gpio_set_dir(POWER_EN, GPIO_OUT); gpio_put(POWER_EN, 1);
    ...
    gpio_init(LCD_RD); gpio_set_dir(LCD_RD, GPIO_OUT); gpio_put(LCD_RD, 1);
    ...
    CMDN(GCTRL,    0x35);
    CMDN(VCOMS,    0x1f);
    CMDN(GMCTRP1,  0xD0, 0x08, 0x11, 0x08, 0x0C, 0x15, 0x39, 0x33, 0x50, 0x36, 0x13, 0x14, 0x29, 0x2D);
    CMDN(GMCTRN1,  0xD0, 0x08, 0x10, 0x08, 0x06, 0x06, 0x39, 0x44, 0x51, 0x0B, 0x16, 0x14, 0x2F, 0x31);
    CMD0(INVON);
    CMD0(SLPOUT);
    CMD0(DISPON);            sleep_ms(100);
...
static void set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    uint8_t cd[4];
    cd[0] = x0 >> 8; cd[1] = x0 & 0xff; cd[2] = x1 >> 8; cd[3] = x1 & 0xff;
    cmd(CASET, 4, cd);
    cd[0] = y0 >> 8; cd[1] = y0 & 0xff; cd[2] = y1 >> 8; cd[3] = y1 & 0xff;
    cmd(RASET, 4, cd);
}
...
void lcd_fill(uint16_t colour) {
    ...
    uint8_t c = RAMWR; wr_blocking(&c, 1);
    gpio_put(LCD_DC, 1);
    static uint16_t line[LCD_W];
    uint16_t be = (colour >> 8) | (colour << 8);
    for (int i = 0; i < LCD_W; i++) line[i] = be;
    for (int y = 0; y < LCD_H; y++) wr_blocking((const uint8_t *)line, LCD_W * 2);
    gpio_put(LCD_CS, 1);
}
...
void lcd_blit_gb(const uint16_t *fb) {
    ...
    if (!colmap_ready) {
        for (int ox = 0; ox < GB_DRAW_W; ox++) colmap[ox] = (ox * GB_W) / GB_DRAW_W;
        colmap_ready = true;
    }
    ...
    for (int oy = 0; oy < GB_DRAW_H; oy++) {
        ...
        for (int ox = 0; ox < GB_DRAW_W; ox++) outline[ox] = srow[colmap[ox]];
