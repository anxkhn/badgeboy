...
All of this lives in `src/config.h`.
...
- Battery-backed saves: persist cart RAM to flash on a timer or on HOME.
- Multi-game launcher: list ROMs and boot the selected one. Decide between
  flash-embedded ROMs and a FatFS plus USB mass-storage workflow.
- Frame pacing and optional overclock for full-speed heavy titles.
- Per-game DMG palette selection.
- NES via a second core (InfoNES) once the launcher exists.
